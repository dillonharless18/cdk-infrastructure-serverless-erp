...
export class CognitoConstruct extends Construct {
    ...
    constructor(scope: Construct, id: string, props: CognitoConstructProps) {
        ...
        if (!domainName)
            throw new Error(
                `Error in cognito stack. domainName does not exist on \n Props: ${JSON.stringify(
                    props,
                    null,
                    2
                )}`
        ...
        const userPool = new cognito.UserPool(this, "UserPool", {
            ...
            signInAliases: {
                // Allow users to sign in using their email address as well as a username
                email: true,
                username: true,
            },
            ...
            autoVerify: {
                // Automatically verify the email address of newly created users
                email: true,
            },
            passwordPolicy: {
        ...
        const adminRolePolicy = JSON.parse(
            fs.readFileSync(
                path.join(__dirname, "..", "iam", "roles", "admin.json"),
                "utf-8"
            )
        );
        const basicUserRolePolicy = JSON.parse(
            fs.readFileSync(
                path.join(__dirname, "..", "iam", "roles", "basic_user.json"),
                "utf-8"
            )
        );
        const logisticsRolePolicy = JSON.parse(
            fs.readFileSync(
                path.join(__dirname, "..", "iam", "roles", "logistics.json"),
                "utf-8"
            )
        );
        const projectManagerRolePolicy = JSON.parse(
            fs.readFileSync(
                path.join(
                    __dirname,
                    "..",
        ...
        const driverRolePolicy = JSON.parse(
            fs.readFileSync(
                path.join(__dirname, "..", "iam", "roles", "driver.json"),
                "utf-8"
            )
        );
        ...
        const adminPolicyDocument =
            iam.PolicyDocument.fromJson(adminRolePolicy);
        const basicUserPolicyDocument =
            iam.PolicyDocument.fromJson(basicUserRolePolicy);
        const logisticsPolicyDocument =
            iam.PolicyDocument.fromJson(logisticsRolePolicy);
        const projectManagerPolicyDocument = iam.PolicyDocument.fromJson(
            projectManagerRolePolicy
        );
        const driverPolicyDocument =
            iam.PolicyDocument.fromJson(driverRolePolicy);

        // Create the roles using the createCustomRole function - NOTE these are intentionally lower case
        const adminRole = createCustomRole(
            this,
            "admin",
            "admin",
            adminPolicyDocument,
        ...
        const basicUserRole = createCustomRole(
            this,
            "basicuser",
            "basicuser",
            basicUserPolicyDocument,
            identityPool.ref
        );
        const logisticsRole = createCustomRole(
            this,
            "logistics",
            "logistics",
            logisticsPolicyDocument,
            identityPool.ref
        );
        const projectManagerRole = createCustomRole(
            this,
            "projectmanager",
            "projectmanager",
            projectManagerPolicyDocument,
            identityPool.ref
        );
        const driverRole = createCustomRole(
            this,
            "driver",
            "driver",
            driverPolicyDocument,
            identityPool.ref
        );
        ...
        const assetBucket = new CustomBucket(
            this,
            `${props.applicationName.toLowerCase()}-${props.stageName}-assets`,
            {
                bucketName: `${props.applicationName.toLowerCase()}-${
                    props.stageName
                }-assets`,
                versioned: false,
        ...
        const basicUserGroup = new cognito.CfnUserPoolGroup(
            this,
            "BasicUserGroup",
            {
        ...
        const logisticsGroup = new cognito.CfnUserPoolGroup(
            this,
            "LogisticsGroup",
            {
                groupName: "logistics",
