...
class VeryfiCustomClient extends Client {
     ...
     */
    async getDocumentsCreatedWithinRange(timeIntervalInMinutes = 15) {
        try {
            const { created__gte, created__lte } =
                this.#getFormattedUTCDateTimeWithDelta(timeIntervalInMinutes);
     ...
     */
    #getFormattedUTCDateTimeWithDelta(timeIntervalInMinutes) {
        const now = new Date();
        const minuteInterval = parseInt(timeIntervalInMinutes, 10);
        ...
        console.log("end of interval", endOfInterval);
