...
import { User } from "aws-cdk-lib/aws-iam/index.js";
  ...
  try {
    ...
    for (const entityObject of constants.entityConstants) {
      ...
      for (const entityName of entityObject.entityNames) {
        const entityInstance = createInstance({ entityName }, adminUserId);
        await insertEntityInstanceIntoDatabase(entityObject.tableName);
      }
      console.log(
