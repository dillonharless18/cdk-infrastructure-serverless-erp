export async function up(knex) {
  ...
  await knex("vendor")
    .whereNull("tax_ID")
    .update({ tax_ID: "desired_value_for_tax_ID" });
  await knex("vendor")
    .whereNull("billed_from")
    .update({ billed_from: "desired_value_for_billed_from" });
  await knex("vendor")
    .whereNull("shipped_from")
    .update({ shipped_from: "desired_value_for_shipped_from" });
  await knex("vendor")
    .whereNull("payment_terms")
    .update({ payment_terms: "desired_value_for_payment_terms" });
  await knex("vendor")
    .whereNull("email")
    .update({ email: "desired_value_for_email" });
  await knex("vendor")
    .whereNull("phone_number")
    .update({ phone_number: "desired_value_for_phone_number" });
  ...
  await knex.schema.alterTable("vendor", function (table) {
    ...
    table.string("payment_terms", 45).notNullable().alter();
    table.string("email", 45).notNullable().alter();
    table.string("phone_number", 45).notNullable().alter();
  });
}
