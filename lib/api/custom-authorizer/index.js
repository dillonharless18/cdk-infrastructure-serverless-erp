...
let pathsToAllowedGroupsList; // Defining this here will store the value between container re-uses
const getPathsToAllowedGroupsList = async () => {
  
  if (!pathsToAllowedGroupsList) { // If the container is re-used we can skip the S3 call
    ...
    pathsToAllowedGroupsList = JSON.parse(configData);
  }

  return pathsToAllowedGroupsList;
};
...
export const handler = async function(event, context) {
  ...
  const pathsToAllowedGroupsList = await getPathsToAllowedGroupsList();
  
  // Extract the object for this path-to-group mapping if it exists - Will be undefined if not
  const pathConfig = pathsToAllowedGroupsList.find(pathcfg => pathcfg.apiPath === requestPath && pathcfg.httpMethod === requestMethod);
