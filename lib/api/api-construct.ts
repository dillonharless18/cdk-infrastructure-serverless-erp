...
export class ApiConstruct extends Construct {
    ...
    constructor(scope: Construct, id: string, props: ApiConstructProps) {
        ...
        if (!domainName)
            throw new Error(
                `Error in API stack. domainName does not exist on \n Props: ${JSON.stringify(
                    props,
                    null,
                    2
                )}`
        ...
        if (!stageName)
            throw new Error(
                `Error in API stack. stageName does not exist on \n Props: ${JSON.stringify(
                    props,
                    null,
                    2
                )}`
        ...
        if (!certficateArn)
            throw new Error(
                `Error in API stack. certificateArn does not exist on \n Props: ${JSON.stringify(
                    props,
                    null,
                    2
                )}`
        ...
        const databaseLayer = new lambda.LayerVersion(this, "DatabaseLayer", {
            code: lambda.Code.fromAsset(
                `${process.env.CODEBUILD_SRC_DIR}/lib/lambda-layers/database-layer`
            ),
            compatibleRuntimes: [lambda.Runtime.NODEJS_18_X],
            description:
                "Exposes packages for db operations: knex, pg, and uuid.",
        });
        ...
        const authorizerLayer = new lambda.LayerVersion(
            ...
            {
                code: lambda.Code.fromAsset(
                    `${process.env.CODEBUILD_SRC_DIR}/lib/lambda-layers/authorizer-layer`
                ),
                compatibleRuntimes: [lambda.Runtime.NODEJS_18_X],
                description:
                    "Exposes packages for authorization operations: aws-jwt-verify.",
            }
        );
        ...
        const errorLayer = new lambda.LayerVersion(this, "ErrorLayer", {
            code: lambda.Code.fromAsset(
                `${process.env.CODEBUILD_SRC_DIR}/lib/lambda-layers/errors-layer`
            ),
            compatibleRuntimes: [lambda.Runtime.NODEJS_18_X],
        ...
        const responseLayer = new lambda.LayerVersion(this, "ResponseLayer", {
            code: lambda.Code.fromAsset(
                `${process.env.CODEBUILD_SRC_DIR}/lib/lambda-layers/response-layer`
            ),
            compatibleRuntimes: [lambda.Runtime.NODEJS_18_X],
        ...
        const backendUtilLayer = new lambda.LayerVersion(
            this,
            "BackendUtilLayer",
            {
                code: lambda.Code.fromAsset(
                    `${process.env.CODEBUILD_SRC_DIR}/lib/lambda-layers/backend-utils-layer`
                ),
                compatibleRuntimes: [lambda.Runtime.NODEJS_18_X],
                description: "Centralizes API response building.",
            }
        ...
        this.responseLambdaLayer = backendUtilLayer;
        ...
        new ARecord(this, "AliasRecord", {
            ...
            target: RecordTarget.fromAlias(
                new ApiGatewayDomain(customDomainName)
            ),
        });
        ...
        const customAuthorizerLambda = new lambda.Function(
            this,
            "AuthorizerLambda",
            {
                ...
                code: lambda.Code.fromAsset(
                    `${process.env.CODEBUILD_SRC_DIR}/lib/api/custom-authorizer`
                ),
        ...
        const lambdasPathFromEnv =
            process.env.CODEBUILD_SRC_DIR_dillonCF_oneXerp_Lambdas_Source;
        if (!lambdasPathFromEnv) {
            console.log(
                "Error: CODEBUILD_SRC_DIR_dillonCF_oneXerp_Lambdas_Source environment variable is not set."
            );
            process.exit(1);
        }
        ...
        const testLambdasPath = path.resolve(
            __dirname,
            "../../test_lambdas/endpoints"
        );
        const functionsPath = fs.existsSync(lambdasPath)
            ? lambdasPath
            : testLambdasPath;
        ...
        const lambdaEndpointsSecurityGroup = new SecurityGroup(
            this,
            "LambdaEndpointsSecurityGroup",
            {
                // vpc: databaseVpc,
                vpc: props.vpc,
        ...
        lambdaEndpointsSecurityGroup.connections.allowTo(
            databaseSecurityGroup,
            Port.tcp(443),
            "Allow Lambda endpoints to access the database"
        );

        // Create the custom authorizer on the API GW and link it to the Lambda
        const customAuthorizer = new apigateway.RequestAuthorizer(
            this,
            "CustomAuthorizer",
            {
        ...
        const authorizationConfigFilePath =
            "./authorizationConfig/authorizationConfig.json";
        fs.writeFileSync(
            authorizationConfigFilePath,
        ...
        new BucketDeployment(this, "DeployConfig", {
            sources: [
                Source.asset(path.dirname(authorizationConfigFilePath), {
        ...
        functionMetadata.forEach((metadata) => {
            const cognitoUserPoolIdMap =
                metadata.name == "createUser" || "updateUser"
                    ? { USER_POOL_ID: props.userPool.userPoolId }
                    : {};
            ...
            const lambdaFunction = new lambda.Function(
                ...
                {
                    code: lambda.Code.fromAsset(
                        path.join(functionsPath, metadata.name)
                    ),
                    handler: "index.handler",
                    runtime: lambda.Runtime.NODEJS_18_X,
                    ...
                    environment: metadata.environment
                        ? {
                              ...
                              RDS_DB_PASS_SECRET_ID:
                                  props.dbCredentialsSecretName.value,
            ...
            const nestedResource = createNestedResource(
                apiV1,
                metadata.apiPath
            );
            ...
            if (mappingTemplateParameters) {
                updatedMappingTemplateParameters = Object.keys(
                    mappingTemplateParameters
                ).reduce((updatedParams, key) => {
            ...
            console.log(
                `Creating method: ${metadata.httpMethod} for path: ${metadata.apiPath}`
            );
...
const getFunctionMetadata = (functionsPath: string) => {
    ...
    const functionMetadata = functionDirectories.map((dir) => {
        ...
        if (fs.existsSync(metadataPath)) {
            ...
            throw new Error(
                `metadata.json file is missing in the '${dir}' Lambda function directory.`
            );
        }
    });

    console.log(
        `functionMetadata: ${JSON.stringify(functionMetadata, null, 2)}`
    );
...
const createNestedResource = (
    ...
    for (const part of pathParts) {
        ...
        if (existingResource) {
            currentResource = existingResource as apigateway.Resource;
            console.log(
                `Existing resource found for path part: ${part}. Using this resource.`
            );
        } else {
            currentResource = currentResource.addResource(part);
            console.log(
                `No existing resource found for path part: ${part}. Creating a new one.`
            );
        }
